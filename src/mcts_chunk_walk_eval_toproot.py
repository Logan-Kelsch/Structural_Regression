...
This no-block version captures any plt.show() calls from FPC fitting and saves
them instead of requiring you to close a GUI window.
"""
...
import legacy.mcts_util as _MU
...
class CaptureMatplotlibShow:
    """
    Capture figures produced by functions that call plt.show().

    This prevents blocking GUI windows during FPC fitting and writes any open
    figures to disk.
    """
...
def fit_fpc_curve_for_chunk(X_ref, chunk_num: int, solver_kwargs: dict, fpc_dir: Path, log):
    ...
    # _E.fit_FPC_part_prop calls plt.show() internally. Capture those figures
    # so this script can run unattended without waiting for a window close.
...
def main():
    ...
    parser.add_argument("--out-name", default=None, help="Optional leaf output folder name")
    parser.add_argument("--out-root-name", default="chunk_walk", help="Top-level output root folder, separate from mcts_runs")
    ...
    # Save under a top-level folder separate from mcts_runs by default:
    #   chunk_walk/<trained_run_name>/chunk_walk_eval_YYYYMMDD_HHMMSS
    # If you pass --out-name, it becomes:
    #   chunk_walk/<trained_run_name>/<out-name>
    out_name = args.out_name or time.strftime("chunk_walk_eval_%Y%m%d_%H%M%S")
    out_root = Path(str(args.out_root_name))
    out_dir = out_root / run_dir.name / out_name
    out_dir.mkdir(parents=True, exist_ok=True)
